...
const NormalQuiz: React.FC<QuizDetailsProps> = ({ currentUserId }) => {
    const [page, setPage] = useState<number>(1);
    const [allQuizzes, setAllQuizzes] = useState<QuizStruct[]>([]);
    const [currQuiz, setCurrQuiz] = useState<QuizStruct | null>(null)
        ...
        const getQuizDetail = async () => {
            ...
            try {
                const result = await axios.get(
                    `http://localhost:3000/game/getAllQuizzes/${currentUserId}/${page}`
                );
                if (result.data.message === "Quizzes fetched successfully") {
                    setAllQuizzes(result.data.allQuizzes);
