...
import { Box, Chip, Grid, Typography, Paper, CircularProgress } from '@mui/material';
...
const PremiumScoreBoard = () => {
  ...
  const [quizDetail, setQuizDetail] = useState<PremiumQuizStruct | null>(null);
  ...
  useEffect(() => {
    const fetchQuizScore = async () => {
      ...
      setError('');
      try {
        const response = await axios.get(`http://localhost:3000/game/getPremiumQuizScore/${quizId}`, {
          headers: {
  ...
  if (loading) return <Box sx={{ textAlign: 'center', padding: '40px' }}><CircularProgress /></Box>;
          ...
          {quizDetail.questions.map((q, questionIndex) => (
            <Paper key={questionIndex} elevation={2} sx={questionBlockStyle}>
              ...
              <Typography color="textSecondary" sx={{ mt: 1, fontWeight: 500 }}>
                <strong>Detailed Description:</strong> {quizDetail.answerDescription[questionIndex]}
              </Typography>
